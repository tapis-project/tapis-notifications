...
# Worker service docker image is created with a unique tag: tapis/<SVC_NAME>-worker-<ENV>-<VER>-<COMMIT>-<YYYYmmddHHMM>
...
# TODO
# TODO review build of worker image
# TODO

# Copy Dockerfiles to build dir
cp Dockerfile Dockerfile.worker $BUILD_DIR
...
TAG_UNIQ2="${REPO}/${SVC_NAME}-worker:${ENV}-${VER}-$(date +%Y%m%d%H%M)-${GIT_COMMIT_LBL}"
TAG_ENV1="${REPO}/${SVC_NAME}:${ENV}"
TAG_ENV2="${REPO}/${SVC_NAME}-worker:${ENV}"
TAG_LATEST1="${REPO}/${SVC_NAME}:latest"
TAG_LATEST2="${REPO}/${SVC_NAME}-worker:latest"
TAG_LOCAL1="${REPO}/${SVC_NAME}:dev_local"
TAG_LOCAL2="${REPO}/${SVC_NAME}-worker:dev_local"
